    ...
    try {
      ...
      if (!response.ok) {
        setError(
          extractApiError(
            payload,
            "The Worker could not generate a tutor response.",
          ),
        );
        setResult(null);
        return;
      ...
      if (!isAskSuccessResponse(payload)) {
        setError({
          ...
          suggestion:
            "The Worker answered, but the migrated UI did not recognize the payload.",
        });
        setResult(null);
        return;
      }
      ...
      setError({
        message,
        suggestion:
          "If the frontend runs on a different origin, point NEXT_PUBLIC_API_BASE_URL at the Worker.",
      });
      setResult(null);
    } finally {
      setIsLoading(false);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
              Socratic tutoring through the Worker boundary
            </h1>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">
              The migrated UI owns the conversation flow, but the actual tutor
              response still comes from the existing Cloudflare Worker and
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
          ...
          {result ? (
            ...
            <p className="mt-5 rounded-[24px] border border-white/10 bg-slate-950/35 p-4 text-sm leading-7 text-slate-300">
              Ask a question to exercise the migrated page against the existing
              Worker tutor route.
            </p>
          )}
        </div>
