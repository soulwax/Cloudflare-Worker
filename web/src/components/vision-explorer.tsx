    ...
    try {
      ...
      if (!response.ok) {
        setError(
          extractApiError(
            payload,
            "The Worker could not classify that image URL.",
          ),
        );
        setActiveStageCount(0);
        return;
      ...
      if (!isVisionSuccessResponse(payload)) {
        setError({
          ...
          suggestion:
            "The Worker returned a payload the migrated UI does not understand yet.",
        });
        setActiveStageCount(0);
        return;
      }
      ...
      setError({
        message,
        suggestion:
          "If the frontend is running on a different origin, set NEXT_PUBLIC_API_BASE_URL to the Worker URL.",
      });
      setActiveStageCount(0);
    } finally {
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
              Typed React UI, Cloudflare Worker inference
            </h1>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">
              This migrated page keeps ResNet-50 execution behind the existing
              Worker boundary. The App Router shell owns the interaction model;
              Workers AI still owns the classification step.
            </p>
          </div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
          ...
          {result ? (
            ...
            <p className="mt-5 rounded-[24px] border border-white/10 bg-slate-950/35 p-4 text-sm leading-7 text-slate-300">
              Submit an image URL to run the migrated page against the Worker AI
              backend.
            </p>
          )}
        </div>
